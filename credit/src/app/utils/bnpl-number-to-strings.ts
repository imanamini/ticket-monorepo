import { priceFormat } from './strings';
...
export const bnplNumberToString = (input: number): string => {
  let output = '';
  powToStr.some(item => {
    if (input >= tenPow(item.pow)) {
      output = bnplNumberToString(Math.floor(input / tenPow(item.pow)));
      output += item.label;
      output += input % tenPow(item.pow) ? separator + bnplNumberToString(Math.floor(input % tenPow(item.pow))) : '';
      return true;
    }
    return false;
  });
