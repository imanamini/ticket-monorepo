...
import { AppComponent } from '@client-monorepo/app-core';
import { browserProfilingIntegration, browserTracingIntegration, captureConsoleIntegration, init } from '@sentry/angular-ivy';
...
if (environment.env === 'production') {
  init({
    ...
    integrations: [browserTracingIntegration(), browserProfilingIntegration(), captureConsoleIntegration({ levels: ['error'] })],
    denyUrls: [
    ...
    beforeSend(event, hint) {
      ...
      const isHttpStatus0Error =
        event.tags?.error_category === 'http_timeout' ||
      ...
      // ===== Console Error Filtering for 422 Status Codes =====
      // Filter out console.error() calls that log 422 status code errors
      if (event.level === 'error' && event.logger === 'console') {
        const consoleMessage = event.message || '';
        const hasBreadcrumbWith422 = event.breadcrumbs?.some(
          (b) => (b.category === 'xhr' || b.category === 'fetch') && b.data?.status_code === 422
        );

        // Check if this is a console error about a 422 status code
        if (hasBreadcrumbWith422 || /status.*422|422.*status/i.test(consoleMessage)) {
          return null; // Don't send console errors related to 422 to Sentry
        }
      }
      ...
      if (error && typeof error === 'object' && 'status' in error) {
        ...
        if (isDigiPayAPI) {
          ...
          // RULE 2: Allow 4xx errors ONLY if response status is 1000 OR critical auth errors (2001, 2002)
          if (httpStatus >= 400 && httpStatus < 500) {
            const isCriticalError =
              responseData && (responseData.status === 1000 || responseData.status === 2001 || responseData.status === 2002);
            if (isCriticalError) {
              // Add context for better debugging
              if (event.contexts) {
                event.contexts.apiError = {
