...
import { filter, of, switchMap, tap, catchError, EMPTY, finalize, takeUntil } from 'rxjs';
...
@Component({
  selector: 'wealth-applet-bnpl-as-mix-wallets',
  standalone: true,
  imports: [
    CommonModule,
    NgxButtonComponent,
  ...
  templateUrl: './bnpl-as-mix-wallets.component.html',
  styleUrl: './bnpl-as-mix-wallets.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class BnplAsMixWalletsComponent extends BaseComponent implements OnInit {
  ...
  handleAutoFill(option: AutoFillOption) {
    ...
    if (selectedId === option.id) {
      this.resetSelection();
      this.amountController.setValue('', { emitEvent: true });
      return;
    }
    ...
    this.amountController.setValue(amount.toString(), { emitEvent: true });
  }
