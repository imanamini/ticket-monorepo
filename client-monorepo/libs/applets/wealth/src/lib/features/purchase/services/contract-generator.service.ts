...
export class ContractGeneratorService {
  ...
  async modifyCredit(pdfBytes: ArrayBuffer): Promise<Uint8Array> {
    ...
    if (this.data.signature) {
      try {
        ...
        // Signature image embedding failed, continuing without signature
        console.error(error);
      }
    }
  ...
  async modifyAll(pdfBytes: ArrayBuffer): Promise<Uint8Array> {
    const pdfDoc = await PDFDocument.load(pdfBytes);
    const page1 = pdfDoc.getPage(0);
    const page7 = pdfDoc.getPage(6);
    const page13 = pdfDoc.getPage(12);
    const page15 = pdfDoc.getPage(14);
    const page16 = pdfDoc.getPage(15);
    const page17 = pdfDoc.getPage(16);
    const page18 = pdfDoc.getPage(17);
    const page20 = pdfDoc.getPage(19);
    const page25 = pdfDoc.getPage(24);
    const today = moment(new Date()).locale('fa');
    await this.getFont(pdfDoc);
    ...
    if (this.data.signature) {
      try {
        ...
        pdfDoc.getPage(24).drawImage(signatureImage, {
          x: 385,
          y: 200,
          width: 120,
          height: 50,
  ...
  async getContracts(symbol: string): Promise<{
    creditContract: Blob;
    ...
    await this.getSejamInfo(symbol);
    ...
    const generalContract = await this.modifyAll(pdfsBytesFiles.generalContract);

    const contract = new Blob([this.toArrayBuffer(creditContract)], { type: 'application/pdf' });
    const customer = new Blob([this.toArrayBuffer(customerAndBrokerContract)], {
      type: 'application/pdf',
    });
    const buyAndSell = new Blob([this.toArrayBuffer(buyAndSellContract)], {
      type: 'application/pdf',
    });
    const seke2AssuranceFund = new Blob([pdfsBytesFiles.seke2AssuranceFund], {
      type: 'application/pdf',
    });
    const riskStatement = new Blob([this.toArrayBuffer(riskStatementContract)], {
      type: 'application/pdf',
    });
    const general = new Blob([this.toArrayBuffer(generalContract)], {
      type: 'application/pdf',
    });
  ...
  async getSejamInfo(symbol: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.purchaseService.getSejamiProfile(symbol).subscribe({
        next: (sejamInfo) => {
          this.data = sejamInfo.result;
          resolve(sejamInfo.result);
        },
        error: (err) => {
          reject(err);
        },
