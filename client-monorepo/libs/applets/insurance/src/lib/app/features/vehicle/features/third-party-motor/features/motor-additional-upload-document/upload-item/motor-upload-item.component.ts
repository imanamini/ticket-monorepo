...
@Component({
  selector: 'motor-upload-item',
  standalone: true,
  imports: [
    UiUploadImageComponent
  ],
  templateUrl: './motor-upload-item.component.html',
  styleUrl: './motor-upload-item.component.scss'
})
export class MotorUploadItemComponent extends ThirdPartyMotorDirective {
  ...
  public uploadDocument(docFile: File): void {
    ...
    super.addSubscription(this.documentApiService.uploadDocument(
      this.applicationFormId, this.uploadedImageFile(), docFile, this.uploadedImageFile().fileName, this.uploadedImageFile().type, 'Motor')
      .subscribe({
  ...
  uploadImage(docFile: File): void {
    if (this.uploadedImageFile()?.id) {
      super.addSubscription(
        this.documentApiService.removeUploadedDocument(this.uploadedImageFile().id, 'Motor')
          .pipe(
  ...
  private removeUploadedDocument(): void {
    super.addSubscription(this.documentApiService.removeUploadedDocument(this.uploadedImageFile().id, 'Motor').subscribe({
      next: () => {
  ...
  private saveDocumentInStore(id: number): void {
    this.storeService.setStoreData({
      ...
      documents: [...this.storeService.getStoreData().documents, {
        id,
        type: this.uploadedImageFile().type,
        fileName: this.uploadedImageFile().fileName,
        title: this.uploadedImageFile().title,
