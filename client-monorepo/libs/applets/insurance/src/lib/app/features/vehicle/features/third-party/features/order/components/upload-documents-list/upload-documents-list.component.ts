...
import { UploadDocumentGuideBottomSheetComponent } from '../upload-document-guide-bottom-sheet/upload-document-guide-bottom-sheet.component';
import { InsuranceNoticeComponent } from '../../../../../../../../components/insurance-notice/insurance-notice.component';
import { catchError } from 'rxjs/operators';
import { HttpResponse } from '@angular/common/http';
import { BaseComponent } from '../../../../../../../../components/base/base.component';
import { UiUploadImageComponent } from '../../../../../../../../components/ui-upload-image/ui-upload-image.component';
import { InsIconComponent } from '../../../../../../components/ins-icon/ins-icon.component';
import { UploadedDocumentModel } from '../../../../../../data-access/models/third-party/upload-document/uploaded-document.model';
...
import { UploadDocumentSettingsResponse } from '../../../../../../data-access/models/third-party/upload-document/upload-document-settings.response';
...
@Component({
  selector: 'upload-documents-list',
  standalone: true,
  imports: [UiUploadImageComponent, InsIconComponent, KeyValuePipe, NgxPlateComponent],
  templateUrl: './upload-documents-list.component.html',
  styleUrl: './upload-documents-list.component.scss',
})
export class UploadDocumentsListComponent extends BaseComponent implements OnInit {
  ...
  topDescription = input<string>(null);
  ...
  loadData(): void {
    super.addSubscription(
      forkJoin({
        requiredFiles: this.appFormApi.getRequiredFiles(this.applicationFormId()),
  ...
  initializeComponentData(): void {
    if (this.updateInputsValidationNotify()) {
      super.addSubscription(
        this.updateInputsValidationNotify().subscribe({
          next: () => {
  ...
  initializeUploadedImages(docs: UploadedDocumentModel[]): void {
    docs
      ?.filter((doc) => !doc.fileName?.includes('other'))
      .forEach((doc: UploadedDocumentModel) => {
    ...
    super.addSubscription(
      this.storeService.getStoreDataAsObservable().subscribe({
        next: (response) => {
          ...
          const uploadedDocs: UploadedDocumentModel[] =
            response.documents?.filter((doc) => doc.documentState === DocumentStateEnum.Uploaded) ?? [];
  ...
  openGuideBottomSheet(): void {
    this.bottomSheetService.open(BottomSheetBoxComponent, {
      component: UploadDocumentGuideBottomSheetComponent,
      name: 'UploadDocumentGuideBottomSheet',
      title: 'راهنمای تصاویر مدارک ',
    });
  }

  deleteImage(doc: UploadedDocumentModel, directDelete = false): void {
    if (directDelete) {
  ...
  removeUploadedDocument(fileName: string): void {
    super.addSubscription(
      this.documentApiService.removeUploadedDocument(this.uploadedImageFiles()[fileName].id).subscribe({
        next: () => {
          this.uploadedImageFiles.update((x) => ({
            ...x,
            [fileName]: {
    ...
    if (this.isUploadedBefore(doc.fileName)) {
      super.addSubscription(
        this.documentApiService
          .removeUploadedDocument(this.uploadedImageFiles()[doc.fileName].id)
          .pipe(
  ...
  private uploadDocument(applicationFormId: string, docFile: File, doc: UploadedDocumentModel): void {
    this.documentApiService.uploadDocument(applicationFormId, doc, docFile).subscribe({
  ...
  areNeededImagesUploaded(): boolean {
    return Object.keys(this.uploadedImageFiles()).every(
      (key) => this.uploadedImageFiles()[key].documentState === DocumentStateEnum.Uploaded,
    );
  }
}
