...
export class PolicyCardThirdPartyCarService extends PolicyCardService {
    ...
    return new Promise(resolve => {
      switch (data.state) {
        ...
        case VEHICLE_POLICY_STATE_ENUM.ISSUED:
          ...
          this.apiService.downloadPolicyThirdparty(uniqueCode).subscribe({
            next: data => {
        ...
        case VEHICLE_POLICY_STATE_ENUM.WAITING_FOR_ADDRESS_REGISTRATION:
          this.router.navigate([`${INSURANCE_APP_PREFIX}/${InsuranceUrlsEnum.VehicleThirdParty}/order/verify-postal-code`], {
            queryParams: {
              [ThirdPartyKeysEnum.FormId]: data.id
            }
          });
  ...
  handleDetailButtonClicked(data: PolicyProductCardModel<PolicyCardTypeModel>): void {
    this.router.navigate([`${INSURANCE_APP_PREFIX}/${InsuranceUrlsEnum.PolicyDetail}`], {
      queryParams: {
        [ThirdPartyKeysEnum.FormId]: data.id,
        [InsuranceKeysEnum.POLICY_TYPE]: data.type
      }
    });
  }
