...
export class PolicyDetailThirdPartyCarService extends PolicyDetailService {
  ...
  getPolicyDetail(id: string): Promise<SectionCardModel[]> {
    return new Promise((resolve, reject) => {
      this.policyApiService.getPolicyDetail(id, null).subscribe({
        next: (response) => {
  ...
  majorActionButtonHandler(): void {
    switch (this.state) {
      ...
      case VEHICLE_POLICY_STATE_ENUM.CANCELLED:
        this.router.navigate([`${INSURANCE_APP_PREFIX}/${InsuranceUrlsEnum.VehicleThirdParty}`]);
        break;
      case VEHICLE_POLICY_STATE_ENUM.PRICE_CONFLICT_RESOLVE_PENDING:
        this.applicationFormPaymentService.postPaymentRequest(this.id, PaymentRequestTypeEnum.CONFLICT);
        break;
      case VEHICLE_POLICY_STATE_ENUM.DOCUMENTS_CONFLICT_RESOLVE_PENDING:
        this.router.navigate([`${INSURANCE_APP_PREFIX}/${InsuranceUrlsEnum.VehicleThirdParty}/order/additional-upload-document`], {
      ...
      default:
        this.router.navigate([`${INSURANCE_APP_PREFIX}/${InsuranceUrlsEnum.VehicleThirdParty}/order/state`], {
          queryParamsHandling: 'preserve',
        });
        break;
    }
  ...
  downloadPolicy(uniqueCode: string | number): void {
    this.policyApiService.downloadPolicyThirdparty(uniqueCode as string).subscribe({
      next: (data) => {
  ...
  getVehicleDetails(data: ApplicationFormGetResponseModel | MotorPolicyResultModel): SectionCardModel {
    const thirdPartyData: ApplicationFormGetResponseModel = data as ApplicationFormGetResponseModel;
    return {
      title: 'مشخصات خودرو',
      card: {
        title: thirdPartyData.vehicleInfo?.fullName ?? this.nullDate,
        subtitle: thirdPartyData?.license ? PlateUtils.convertCarToText(thirdPartyData.license) : undefined,
        icon: thirdPartyData.vehicleInfo?.carBrandLogo ? 'car-' + thirdPartyData.vehicleInfo?.carBrandLogo : IconEnum.DefaultCar,
        expandable: true,
        details: [
          {
            type: 'text',
            title: 'تیپ',
            value: thirdPartyData.vehicleInfo?.carModel ?? this.nullDate,
            ellipsis: true,
          },
          {type: 'text', title: 'کاربری', value: thirdPartyData.vehicleInfo?.carUsage ?? this.nullDate},
          {
            type: 'text',
            title: 'سال ساخت',
            value: thirdPartyData.vehicleInfo?.carBuildYear?.toString() ?? this.nullDate,
          },
        ],
      },
    };
  ...
  setUiRelatedProperties(data: ApplicationFormGetResponseModel): void {
    switch (data.state.displayState) {
      ...
      case VEHICLE_POLICY_STATE_ENUM.PRICE_CONFLICT_RESOLVE_PENDING:
        ...
        this.infoText = `کاربر گرامی، به دلیل مغایرت اطلاعات خوداظهاری وارد شده توسط شما با اطلاعات اسناد بارگذاری شده، و بررسی آن توسط کارشناس بیمه دیجی‌پی، برای صدور بیمه‌نامه نهایی خود، تا ۷۲ ساعت مهلت دارید تا نسب به پرداخت مبلغ مابه‌التفاوت`;
        this.alertColor = AlertColorEnum.Red;
        break;
      case VEHICLE_POLICY_STATE_ENUM.EXPIRING:
