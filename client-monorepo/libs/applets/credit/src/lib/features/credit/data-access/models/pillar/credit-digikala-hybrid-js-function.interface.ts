import { NavigationTabState } from '../../services/pillar/model/credit-digikala-hybrid-js-function.interface';

export type HeaderState = 'compact' | 'full';
export type Platform = 'android' | 'ios';
export type HeaderChangeState = 'expand' | 'hide' | 'compact';

interface ICreditDigikalaJsFunction {
  /**
  ...
  /**
   * only open digikala links inside super app
   * @param url - The URL to open
   */
  ...
  /**
   * ask user to grant camera access and call the given callback
   * @param cbFn - Callback function with granted: boolean
   */
  requestCameraPermission(cbFn: (granted: boolean) => void): void;

  /**
   * Ask user to grant image gallery access and call the given callback
   * @param cbFn - Callback function with granted: boolean
   */
  requestImagePermission(cbFn: (granted: boolean) => void): void;

  /**
   * Ask user to grant record audio access and call the given callback
   * @param cbFn - Callback function with granted: boolean
   */
  requestRecordAudioPermission(cbFn: (granted: boolean) => void): void;

  /**
   * Opens a url in the digikala super app
   * @param url - The URL to open
   */
  openLink(url: string): void;

  /**
   * Retrieve the current saToken (async)
   * @returns null or the token string
   */
  getSuperAppTokenAsync(): Promise<string>;
}

interface IDigikalaSuperWebJsFunction {
  isSuperWebUser(): boolean;
  navigationTabState: NavigationTabState;
}

export interface CreditDigikalaAppWindow extends Window {
  SuperAppApi: ICreditDigikalaJsFunction;
}

export interface DigikalaSuperWebWindow extends Window {
  SuperWebSDK: IDigikalaSuperWebJsFunction;
}
