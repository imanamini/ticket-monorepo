...
export class CacheService {
  ...
  public async addToCache(url: string, response: any, ttl = 0): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const data = { url, response, ttl: computedTtl, status: 'done'};
  ...
  async deleteFromCache(url: string, isHashedBefore = true): Promise<void> {
    ...
    if (isHashedBefore) {
      process = store.delete(url);
    } else {
      process = store.delete(this.makeUrlIndex(url));
    }
    return new Promise<void>((resolve, reject) => {
      process.onsuccess = () => resolve();
      process.onerror = () => reject(process.error);
    });
  }
