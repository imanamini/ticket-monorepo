...
@Component({
  selector: 'app-test-digiplus-direct-debit-v2',
  templateUrl: './test-digiplus-direct-debit-v2.component.html',
  styleUrls: ['./test-digiplus-direct-debit-v2.component.scss']
})
export class TestDigiplusDirectDebitV2Component {
  ...
  public state: DirectDebitGenerateTicketBody = {
    ...
    duration: {
      timeUnit: this.DurationTimeUnitEnum.YEAR,
      count: 1
    }
  };
  ...
  public generateTicket(): void {
    this.loading = true;
    this.directDebitApiV2Service.getDirectDebitTicket(this.state, {headers: new HttpHeaders().set('Authorization', 'Bearer ' + this.token)})
      .subscribe(
        response => {
          this.loading = false;
          this.router.navigateByUrl('/direct-debit-v2/external/withdrawal-detail/' + response.ticket).then();
        },
