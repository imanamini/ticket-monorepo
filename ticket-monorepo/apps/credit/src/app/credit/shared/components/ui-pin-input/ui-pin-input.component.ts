...
import { PinLength } from './data-access/pin-length';
import { OtpLength } from './data-access/otp-length';
...
@Component({
  ...
  styleUrl: './ui-pin-input.component.scss'
})
export class UiPinInputComponent implements OnInit, OnChanges {
  ...
  pinInputs = new Array(PinLength);

  otpInputs = new Array(OtpLength);
  ...
  dynamicFocusInput(index: number) {
    if (index < 0 || index > (Math.max(OtpLength, PinLength) - 1)) {
      return;
    }
